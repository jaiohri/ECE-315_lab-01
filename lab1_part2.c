...
#define PUSHBTN_DEVICE_ID   XPAR_GPIO_BTNS_BASEADDR
#define PUSHBTN_CHANNEL   1
...
{
    const u8 color = RGB_CYAN;
    const u32 PWM_PERIOD = 100;
    const u32 DUTY_STEP = 10;
    TickType_t xOnDelay = PWM_PERIOD / 2;
    TickType_t xOffDelay = PWM_PERIOD - xOnDelay;

    while (1) {
        u32 pb = XGpio_DiscreteRead(&PushButtons, PUSHBTN_CHANNEL);
        if (pb == 8) {
            if (xOnDelay + DUTY_STEP <= PWM_PERIOD - 1) {
                xOnDelay += DUTY_STEP;
                xOffDelay = PWM_PERIOD - xOnDelay;
                xil_printf("\nDuty cycle increased: Ton=%u, Toff=%u\n",
                    (unsigned)xOnDelay, (unsigned)xOffDelay);
                vTaskDelay(pdMS_TO_TICKS(200));
            }
        } else if (pb == 1) {
            if (xOnDelay >= DUTY_STEP + 1) {
                xOnDelay -= DUTY_STEP;
                xOffDelay = PWM_PERIOD - xOnDelay;
                xil_printf("\nDuty cycle decreased: Ton=%u, Toff=%u\n",
                    (unsigned)xOnDelay, (unsigned)xOffDelay);
                vTaskDelay(pdMS_TO_TICKS(200));
            }
        }

        XGpio_DiscreteWrite(&RgbLed, RGB_CHANNEL, color);
        vTaskDelay(xOnDelay);
...
/*************************** Enter your code here ****************************/
    ...
    XGpio_Initialize(&RgbLed, RGB_LED_BASEADDR);
    XGpio_SetDataDirection(&RgbLed, RGB_CHANNEL, 0);
    XGpio_Initialize(&PushButtons, PUSHBTN_DEVICE_ID);
    XGpio_SetDataDirection(&PushButtons, PUSHBTN_CHANNEL, 0xFFFFFFFF);
    xTaskCreate(vRgbTask, "RGB task", configMINIMAL_STACK_SIZE, NULL, tskIDLE_PRIORITY, NULL);
